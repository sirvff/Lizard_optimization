...
#define MIN_A_BYTES (12 * (PublicKey_A >> 4)) // Here we will use only 3/4 of the PublicKey_A 
unsigned char A_bytes[MIN_A_BYTES];          //since we need 3 random bytes to produce 2 16-bit values
#endif
int count = 0;
...
#else
uint16_t Sample_D1() {
	if (count == LWE_M * LWE_L1) count = 0; // If we are at the end of the array 'seed', start from 0
	uint16_t rnd = seed[count] & 0x01ff;  // We use single uint16_t for both rnd and sign (we need only 10 bits for them)
	uint16_t sign = (seed[count++] & 0x0200) >> 9; // In the original version, we used separate uint16_t values for rnd and sign
	uint16_t sample = 0;    // The rest remains unchanged
	for (size_t i = 0; i < TABLE_LENGTH - 1; ++i) {
		sample += (CDF_TABLE[i] - rnd) >> 15; 
	}
	sample = ((-sign) ^ sample) + sign;
...
#else  // removed  <<= _16_LOG_Q in the loop, the rest has not changed
	for (i = 0; i < LWE_M; ++i) {
		for (j = 0; j < LWE_N; ++j) {
			pk[(i * LWE_N * 2) + (j * 2)] = pk_A[i * LWE_N + j] >> 8;
			pk[(i * LWE_N * 2) + (j * 2 + 1)] = pk_A[i * LWE_N + j] & 0xff;
...
#else
randombytes(sk_t, LWE_N * LWE_L1 / 4); // sk[i] is defined by 2 last bits of sk_t[i], so we need only 2 random bits for calculation of each sk[i]
register uint8_t shift0; // The required bits are taken from the byte with index i / 4 (each byte from sk_t is used for 4 sk bytes) 
for (i = 0; i < LWE_N * LWE_L1; ++i) {
	// Take the byte sk_t[i/4], from this byte take 2 bits starting from left, i.e. i=0 -> bits 8 and 7; i=1 - bits 6 and 5, etc.
	// We get these bits by shifting the byte to the right by (3-i%4)*2 bits, then applying the mask 0x03 - the rightmost 2 bits.
	shift0 = ((sk_t[i » 2] »((3 - (i & 3)) « 1)) & 0x03);
	if (shift0 == 0x00) // Now the same as in the original version
		sk[i] = -1;
	else if (shift0 == 0x01)
		sk[i] = 1;
	else
...
#else
// All variables now 'register', otherwise, the code has not changed
...
// two additional variables allow only adding 1 and 2 instead of many mathematical operations in the loop body
	...
	for (i = 0; i < LWE_M; ++i) {
		for (j = 0; j < LWE_L1; ++j) {
			*pki = *pkAi >> 8; //pk[PublicKey_A + (i * LWE_L1 * 2) + (j * 2)] = pk_B[i * LWE_L1 + j] >> 8;
			pki[1] = *pkAi & 0xff; //pk[PublicKey_A + (i * LWE_L1 * 2) + (j * 2 + 1)] = pk_B[i * LWE_L1 + j] & 0xff;
			pkAi++; // move to next pk_B
			pki += 2; //move to next pk
		}
	}
#endif
step5 += (clock() - start1);
...
//start1 = clock();
...
#else
	// register variables and pointers are entered instead of the original ones
	...
	for (k = 0; k < LWE_L2; ++k) {
		...
		neg_start[k] = neg;
		r_t += LWE_M;   // instead of multiplying k * LWE_M now  rt += LWE_M
...
//start1 = clock();
	...
	for (i = 0; i < LWE_M * LWE_L1; i++)
	{
		...
		*pkA = *pka++; 
		// pk_A[i * LWE_N + j] = pk[(i * LWE_N * 2) + (j * 2)] << 8 | pk[(i * LWE_N * 2) + (j * 2 + 1)];
		pkA += 2; // move to next pk_A

		*(pkB + 1) = *pkb++; 
		*pkB = *pkb++; 
		// pk_B[i * LWE_L1 + j] = pk[PublicKey_A + (i * LWE_L1 * 2) + (j * 2)] << 8 | pk[PublicKey_A + (i * LWE_L1 * 2) + (j * 2 + 1)];
		pkB += 2; //move to next pk_B
	}
...
//start1 = clock();
	...
	for (k = 0; k < LWE_L2; k++) {
		negk = -neg_start[k];
		for (i = 0; i < HR; ++i) {
			uint16_t* pk_A_ri = pk_A + LWE_N * (*r_idx_i);    //this line is the same as the original
			uint16_t* pk_B_ri = pk_B + LWE_L1 * (*r_idx_i++); //this line is the same as the original
			
			if ((negk++) >> sft & 0x1)
			{
				for (j = 0; j < LWE_L1; ++j) {
					c1[k * LWE_N + j] += pk_A_ri[j];  // c1[k * LWE_N + j] += branch * pk_A_ri[j]; branch = 1
					c2[k * LWE_L1 + j] += pk_B_ri[j]; // c2[k * LWE_L1 + j] += branch * pk_B_ri[j]; branch = 1
				}
				for (j = LWE_L1; j < LWE_N; ++j) {
					c1[k * LWE_N + j] += pk_A_ri[j]; // c1[k * LWE_N + j] += branch * pk_A_ri[j]; branch = 1
				}
...
int crypto_kem_dec(unsigned char *ss, unsigned char *ct, unsigned char *sk) {
	...
	int dv_cur = 0; int dv_curj = 0; int dv_cur2 = 0;
...
#else
	register unsigned char *pka = sk + CRYPTO_SECRETKEYBYTES;
	register unsigned char *pkb = sk + CRYPTO_SECRETKEYBYTES + PublicKey_A;
	register unsigned char *pkA = (unsigned char*)pk_A;
	register unsigned char *pkB = (unsigned char*)pk_B;
	for (i = 0; i < LWE_M; i++)
	{
...
#else
	register uint16_t *r_idx_i = r_idx;
	register int negk;
	for (k = 0; k < LWE_L2; k++) {
		negk = -neg_start[k];
		for (i = 0; i < HR; ++i) {
			...
			{
				for (j = 0; j < LWE_L1; ++j) {
					c1[k * LWE_N + j] += pk_A_ri[j];
					c2[k * LWE_L1 + j] += pk_B_ri[j];
				}
				for (j = LWE_L1; j < LWE_N; ++j) {
					c1[k * LWE_N + j] += pk_A_ri[j];
				}
